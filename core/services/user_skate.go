...
func (store *userSkateStore) GetUserSkatesNotBeingSharpened(userId int) ([]models.UserSkateStruct, error) {
	sql := `SELECT 
			...
			INNER JOIN brands as steel_brand ON user_skates.steel_id = steel_brand.id
			LEFT JOIN colour as guard_colour ON user_skates.guard_colour_id = guard_colour.id
			WHERE user_id = ? AND user_skates.id NOT IN (SELECT user_skate_id FROM open_sharpenings)`

	query, err := store.database.Tx.Prepare(sql)
	if err != nil {
		store.log.WithFields(logrus.Fields{
			"event":      "userSkateStore::GetUserSkatesNotBeingSharpened - Failed to prepare GetUserSkatesNotBeingSharpened SELECT query.",
			"stackTrace": string(debug.Stack()),
		}).Error(err)
		return nil, err
	}
